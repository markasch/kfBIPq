...
$$
where $\mathcal{G}$ denotes the forward model from inputs to observables, $\eta \sim \mathcal{N}(0, \Sigma_{\eta})$ represents the model error and observation noise, with $\Gamma$ a positive definite noise covariance matrix.
